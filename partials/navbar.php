...
<header id="header" class="header h-16 fixed top-0 left-0 w-full bg-light flex justify-between items-center px-[5vw] md:px-[10vw] z-30">
  ...
  <nav class="md:inline-flex items-center hidden">
    ...
    <?php if(isset($_SESSION['loggedin']) == true) {?>
      <a href="<?= $BASE_DIR ?>loguser.php" class="app-button px-4 py-2 rounded-3xl bg-primary text-light bg-gradient-to-r from-[#00000044] to-transparent ms-3">Log out</a>
...
<aside class="sidebar left-0 h-screen w-full fixed top-0 z-40 bg-light left-[-105vw]" style="transition: left .5s ease;" id="sidebar">
  ...
  <?php if(isset($_SESSION['loggedin']) == true) {?>
    ...
    <?php if(isset($_SESSION['loggedin']) == true) {?>
      <a href="<?= $BASE_DIR ?>login.php" class="app-button px-4 py-2 rounded-3xl bg-primary text-light bg-gradient-to-r from-[#00000044] to-transparent ms-3">Log out</a>
    <?php } else{ ?>
